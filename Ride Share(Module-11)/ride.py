...
class Ride:
    ...
    def set_driver(self, driver): # driver ak ta object
        self.driver = driver
...
class RideRequest:
    def __init__(self, rider, end_location):
        self.rider = rider # rider er ak ta object
        self.end_location = end_location

class RideMatching:
    def __init__(self, drivers):
        self.available_drivers = drivers # drivers ak ta list of drivers

    def find_driver(self, ride_request, vehicle_type): # ride request ek ta object
        if len(self.available_drivers) > 0:
            print("Looking for drivers....")
            driver = self.available_drivers[0]
            
            if vehicle_type == 'car':
                vehicle = Car("car", "ABC123", 30)
            elif vehicle_type == 'bike':
                vehicle = Bike("bike", "123BD", 50)
